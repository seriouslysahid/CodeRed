  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={clsx(
            'fixed z-50 bg-card shadow-lg border border-border focus:outline-none',
          ...
          <div className={clsx(
            ...
            <div className="flex items-start justify-between flex-shrink-0">
              <div className="flex-1 min-w-0">
                {title && (
                  <Dialog.Title className="text-lg sm:text-xl font-semibold text-foreground mb-2 pr-8">
                    {title}
                  </Dialog.Title>
                )}
                {description && (
              ...
              {showCloseButton && (
                <Dialog.Close asChild>
                  <button
                    className="flex-shrink-0 rounded-full p-2 opacity-70 ring-offset-background transition-all hover:opacity-100 hover:bg-accent hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none -mr-2 -mt-2"
